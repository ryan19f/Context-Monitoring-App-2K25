...
object HeartRateHelper {
    ...
    fun computeHeartRateFromVideo(ctx: Context, uri: Uri): Int {
        ...
        if (frames.isEmpty()) {
            Log.w(TAG, "No frames retrieved for uri=$uri")
            return 0
        }
        val spanSec = if (timesUs.size >= 2) (timesUs.last() - timesUs.first()) / 1_000_000.0 else 0.0
        Log.d(TAG, "Sampled ${frames.size} distinct frames over ${"%.2f".format(spanSec)}s")
    ...
    private fun grabFrameAt(retriever: MediaMetadataRetriever, tUs: Long): Bitmap? {
        // Prefer the actual closest frame first; falls back to sync-only when the decoder demands it.
        var bmp = retriever.getFrameAtTime(tUs, MediaMetadataRetriever.OPTION_CLOSEST)
        if (bmp != null) return bmp

        // Many devices only expose sync (I) frames to MMR
        bmp = retriever.getFrameAtTime(tUs, MediaMetadataRetriever.OPTION_CLOSEST_SYNC)
        if (bmp != null) return bmp
